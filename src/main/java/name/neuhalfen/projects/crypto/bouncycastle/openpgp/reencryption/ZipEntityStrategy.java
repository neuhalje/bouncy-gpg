...
/**
 * Strategy for handling the content of an encrypted ZIP file. <p> The data flow is: <p> [encrypted
 * data from e.g test.zip.gpg] * for each file/dir -- [name sanitation] --
 * ZipEntityStrategy::handleDirectory -- e.g. create directory OR -- output :=
 * ZipEntityStrategy::createOutputStream -- e.g. create a FileOutputStream --
...
public interface ZipEntityStrategy {
  ...
  /**
   * Sanitize the names of entities in ZIP files. These can contain absolute paths, and path
   * traversals.
   *
  ...
  /**
   * Handle a directory. The names are guaranteed to have be sanitized by  {@link
   * #rewriteName(String)}
   *
